...
PHPのモノリスで開発されているCSツールの機能要件の実装や、CSツールフルリプレイスを伴うWebフロントエンドのアーキテクチャ設計、技術選定、実装を担当。また、[メルカリWeb版](https://jp.mercari.com/)の機能要件の実装やリアーキテクチャにも携わり、Backend Web PlatformチームやWeb DevOpsチームに所属し、開発ツールやデプロイツール、エラートラッキング、Kubernetesマニフェスト、CI/CD、IaCなどに責務を持ち従事。
...
- ピープルマネジメント
  - 採用に関わり、チームやプロジェクトが必要とする人材の定義および選考が可能です。定期的な1on1ミーティングによる対話での基本的なモチベーションマネジメントをはじめとして、チームやプロジェクトの問題やメンバーの業務の阻害要因の早期発見と解決ができます。また、チームやメンバーのために必要であれば組織で未導入の制度やルールを設計してチーム内で適用した実績があります。
- プロジェクトマネジメント
...
ソフトウェアエンジニアとして、Webフロントエンドに関わるアーキテクチャ設計や技術選定およびインフラやCI/CDパイプラインまで含めた一気通貫での設計、実装、構築が可能です。また、小中規模のWebシステム全体のアーキテクチャ設計から技術選定および実装を行うことができます。
...
Lerna v4 v5, webpack v1~, Rollup, Vue v2, Vuex, CircleCI, GitHub Actions, Spinnaker, React v0.13~, Electron v1, Next.js v5 v9, Docker, Kubernetes, MySQL, Redis, Apollo Client, Ruby on Rails v4 v5, Gin, Firebase Realtime Database, Firestore, Firebase Hosting, Terraform v0.12~, Nuxt v2, nginx, Redux, Prisma, Express, EC2, S3, CloudFront, Lambda, GCS, GAE, GCR, Cloud Build, GKE, Datadog, BugSnag, Sentry
